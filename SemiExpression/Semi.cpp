	...
	*/
	...
	void Semi::trans() {
		...
		for (Token tok : toks)
		{
			...
			{
				size_t pos1 = tok.find_last_of("\"");
				if (pos1 <= tok.size())
					tok = tok.substr(0, pos1);
				size_t pos2 = tok.find_first_of("\"");
				if (pos2 >= 0)
					tok = tok.substr(pos2 + 1, tok.size());
			}
		}
	}
	*/
...
int main()
{
	...
	std::string source = "../DirExplorer-Naive/DirExplorerN.h";
